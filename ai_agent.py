...
if _HAS_TORCH:
    class BehaviorCloningModel(nn.Module):
        def __init__(self, num_buttons=32):
            ...
            self.fc_axes = nn.Linear(256, 4)
            self.fc_buttons = nn.Linear(256, num_buttons)
        ...
        def forward(self, x):
            ...
            axes = torch.tanh(self.fc_axes(x))
            buttons = self.fc_buttons(x)
...
class AIManager(QObject):
    ...
    def __init__(self):
        ...
        if not _HAS_TORCH:
            raise ImportError(
                "PyTorch is not installed. AI features require: pip install torch torchvision\n"
                "For CPU-only builds, install with: pip install torch torchvision --index-url https://download.pytorch.org/whl/cpu"
            )
    ...
    def _train_loop(self):
        ...
        X = torch.tensor(np.array(self.data_frames), dtype=torch.float32).unsqueeze(1).to(self.device)
